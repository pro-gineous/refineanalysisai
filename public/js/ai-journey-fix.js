// AI Journey Fix - Scripts para reparar errores de sintaxis y visualizaciones

// Función para inicializar visualizaciones después de que la página cargue completamente
function initAIJourneyVisualizations() {
    console.log('Inicializando visualizaciones de AI Journey...');
    ...
    // Variable global para el framework actual
    let currentFramework = '';
    
    // Si no existen los elementos necesarios, terminar
    if (!frameworkSelector || !generateProcessMapBtn || !generateDataModelBtn || !visualizationArea) {
        console.warn('Elementos de visualización no encontrados');
        return;
    }
    
    // Inicializar el selector de framework
    frameworkSelector.addEventListener('change', function() {
        currentFramework = this.value;
        console.log('Framework seleccionado:', currentFramework);
        
        // Ocultar todos los contenidos de framework
        document.querySelectorAll('.framework-content').forEach(content => {
            content.classList.add('hidden');
        });
        
        // Mostrar el contenido del framework seleccionado
        const selectedContent = document.getElementById(`${currentFramework}-content`);
        if (selectedContent) {
            selectedContent.classList.remove('hidden');
        } else {
            const noFrameworkMessage = document.getElementById('no-framework-message');
    ...
    // Inicializar botón de mapa de procesos
    generateProcessMapBtn.addEventListener('click', function() {
        ...
        setTimeout(function() {
            ...
            // Generar el mapa de procesos según el framework
            ...
            // Mostrar el mapa de procesos
            visualizationArea.innerHTML = processMap;
            
            // Añadir mensaje de éxito si existe la función addMessage
            if (typeof window.addMessage === 'function') {
                window.addMessage('ai', '<div class="bg-green-50 border-l-4 border-green-500 p-4"><p class="text-green-700">Mapa de procesos generado exitosamente.</p></div>', true);
            }
        }, 1000);
    });
    
    // Inicializar botón de modelo de datos
    generateDataModelBtn.addEventListener('click', function() {
        ...
        setTimeout(function() {
            ...
            // Añadir mensaje de éxito si existe la función addMessage
            if (typeof window.addMessage === 'function') {
                window.addMessage('ai', '<div class="bg-green-50 border-l-4 border-green-500 p-4"><p class="text-green-700">Modelo de datos generado exitosamente.</p></div>', true);
            }
        }, 1000);
    });
}

// Funciones para generar SVGs de visualización

// Función para generar mapa de procesos PRINCE2
...
// Función para generar mapa de procesos de gestión de ideas
...
// Función para generar mapa de procesos genérico
...
// Función para generar modelo de datos
...
window.addEventListener('DOMContentLoaded', function() {
    ...
    window.createPRINCE2ProcessMap = createPRINCE2ProcessMap;
    window.createIdeaManagementProcessMap = createIdeaManagementProcessMap;
    window.createGenericProcessMap = createGenericProcessMap;
    window.createDataModel = createDataModel;
    
    // Inicializar visualizaciones
    initAIJourneyVisualizations();
    
    console.log('AI Journey Fix cargado exitosamente');
});
